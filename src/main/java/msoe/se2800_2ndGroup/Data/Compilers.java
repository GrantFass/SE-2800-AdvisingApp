 ...
 * * Compiling data that is in multiple forms and or multiple locations into one format and
 * returning it
 * Modification Log:
 ...
 * * Fix output of duplicate course codes in course recommendations due to electives being
 * satisfied by Grant Fass on Thu, 22 Apr 2021
 ...
 * * Finish implementation to compile graduation plans as AcademicTerm objects by Grant Fass on
 * Wed, 5 May 2021
 * * Moved graduation plan compilation methods to their own class by Grant Fass on Sun, 9 May 2021
 * <p>
 * Copyright (C): TBD
 *
 * @author : Grant
...
public class Compilers {
     ...
     * <p>
     * This method compares the list of all offerings in a term against the list of unsatisfied
     * courses and looks for matches.
     ...
     * @return the list of the courses that have not yet been satisfied and occur in the
     * specified term
     * @author : Grant Fass
        ...
        for (CurriculumItem curriculumItem : unsatisfiedCourses) {
            for (Offering offering : offeringsInTerm) {
                if (!(curriculumItem instanceof Elective) &&
                    curriculumItem.satisfiedBy(offering.course())) {
                    LOGGER.finer("Adding Course: " + curriculumItem);
                    outputCourses.add(offering.course());
                } else if (curriculumItem instanceof Course) {
                    LOGGER.finest(String.format("Course %s does not match Offering %s",
                                                ((Course) curriculumItem).code(),
     ...
     * Once the correct major is found it uses the getUnsatisfiedItems method to retrieve a list
     * of the courses
     * that have not yet been completed.
     *
     * @param completedCourses the list of already completed courses, usually from the loaded
    ...
    public static List<CurriculumItem> getCurriculaExcludingCompletedCourses(
        ...
        throw new CustomExceptions.InvalidCurriculaException(
                "Curriculum for selected major not found");
    }
     ...
     */
    public static ArrayList<Offering> getCourseOfferings(boolean displayFall, boolean displayWinter,
    ...
    throws CustomExceptions.InvalidInputException {
        ...
        try {
            ...
            for (Offering offering : Data.getOfferings()) {
                ...
                } else if (offering.availability().get(Data.getMajor()).equals(Term.WINTER) &&
                           displayWinter) {
                    offeringsBySeason.add(offering);
                } else if (offering.availability().get(Data.getMajor()).equals(Term.SPRING) &&
                           displaySpring) {
                    offeringsBySeason.add(offering);
                }
            }
            ...
            LOGGER.log(Level.WARNING, String.format(
                    "The specified major %s was not found which means it was not input correctly",
                    Data.getMajor()), e);
    ...
    /**
     * This method processes a list of curriculum items and determines if the items contained
     * within have their prerequisites satisfied
     *
     ...
     * @return the list of items only containing courses that have satisfied prerequisites and
     * electives
     * @author : Grant Fass
     * @since : Thu, 22 Apr 2021
     */
    public static List<CurriculumItem> getCurriculumItemsWithSatisfiedPrerequisites(
            List<CurriculumItem> curriculumItems, Collection<String> completedCourseCodes) {
        ...
        for (CurriculumItem curriculumItem : curriculumItems) {
            if ((curriculumItem instanceof Course) &&
                ((Course) curriculumItem).prerequisite().satisfiedBy(completedCourseCodes) ||
                (curriculumItem instanceof Elective)) {
     ...
     * <p>
     * This method first verifies that the Major, Offerings, and Transcript have been loaded and
     * are not empty.
     * This method then computes the offerings available.
     * This method then finds the list of courses not yet satisfied in the curriculum and
     * compares it against the offerings.
     *
     ...
     * @since : Thu, 22 Apr 2021
     */
    public static List<CurriculumItem> getCourseRecommendation(boolean getFall, boolean getWinter,
                                                               boolean getSpring)
    throws CustomExceptions.InvalidInputException {
        Data.verifyMajor();
        Data.verifyOfferings();
        ...
        List<CurriculumItem> uncompletedCurriculumCourses =
                getCurriculaExcludingCompletedCourses(Data.getTranscriptCourses());
        List<CurriculumItem> unsatisfiedCurriculumItems =
                getUnsatisfiedCurriculumItemsForSpecifiedTerm(offeringsForTerm,
                                                              uncompletedCurriculumCourses);
        return getCurriculumItemsWithSatisfiedPrerequisites(unsatisfiedCurriculumItems, Manipulators
                .getCourseCodes(Data.getTranscriptCourses()));
    }
     ...
     * <p>
     * Sources:
     * <a href="#{@link}">{@link "https://www.w3schools.com/java/java_regex.asp"}</a>: Help
     * determining regex expressions for filtering input String
     *
    ...
    public static String getPrerequisitesForCourse(String course) {
        ...
        LOGGER.finer(String.format("Searching for prerequisites for the course with code: %s",
                                   selected == null ? "null" : selected.code()));

        if (selected != null) {
            if (selected.prerequisite() instanceof AndPrerequisite and) {
                return ("Prerequisites: " + ((SinglePrerequisite) and.left()).code() + " and " +
                        ((SinglePrerequisite) and.right()).code());
            } else if (selected.prerequisite() instanceof OrPrerequisite or) {
                return ("Prerequisites: " + ((SinglePrerequisite) or.left()).code() + " or " +
                        ((SinglePrerequisite) or.right()).code());
